...
tstop1 = initializing_time + (stim_length + stim_delay)*(num_exposures)
tstop2 = initializing_time + (stim_length + stim_delay)*(num_degradations)

bins = np.linspace(0,1, num_bins + 1)
vecstims = [h.VecStim() for i in range(num_neurons)]
evecs = [h.Vector() for i in range(num_neurons)]

vecstims1 = [h.VecStim() for i in range(num_neurons)]
evecs1 = [h.Vector() for i in range(num_neurons)]
vecstims2 = [h.VecStim() for i in range(num_neurons)]
evecs2 = [h.Vector() for i in range(num_neurons)]

# Network Topography
# initialize with no connections
connection_matrix = [[0 for ii in range(num_neurons)] for ii in range(num_neurons)]

# fully connected with self connections
connection_matrix = [[1 for ii in range(num_neurons)] for ii in range(num_neurons)]
# remove self connections
for k in range(num_neurons):
    connection_matrix[k][k] = 0

# sparsely connected network
connection_matrix = [[random.binomial(1, sparsity) for ii in range(num_neurons)] for ii in range(num_neurons)]

cell_list = []
for ii in range(num_neurons):
    pc_ca3 = cell_network.Cell(ID,location,synvars,celltype,fname_morph,num_input,num_neurons,modeltype)
...
complete_input_vector1 = [0]*num_neurons
complete_input_vector2 = [0]*num_neurons
...
for j in range(num_patterns):
    for i in range(num_neurons):
        ...
        while retrieved_pattern[i] < 0:
            retrieved_pattern[i] = true_pattern[i] + random.normal(0, 0.1)
        ...
        if len(true_spikes[i]) != 0:
            true_hanging_interval = stim_length + stim_delay - true_spikes[i][-1]
        else:
        ...
        for k in range(num_degradations):
            ...
            while input_pattern_tmp < 0:
                input_pattern_tmp = old_pattern + random.normal(0, degradation)
        ...
        if len(degraded_spikes_full) != 0:
            degraded_spikes_full[0] = degraded_spikes_full[0] - stim_delay
        ...
        if len(true_spikes_full[i]) != 0:
            ...
            complete_input_vector1[i] = unpatterned_intervals[i] + true_spikes_full[i]
            complete_input_vector2[i] = unpatterned_intervals[i] + degraded_spikes_full
        else:
            complete_input_vector[i] = unpatterned_intervals[i]
            complete_input_vector1[i] = unpatterned_intervals[i]
            complete_input_vector2[i] = unpatterned_intervals[i]
            
        complete_input_spikes = np.cumsum(complete_input_vector[i])[:]
        complete_input_spikes1 = np.cumsum(complete_input_vector1[i])[:]
        complete_input_spikes2 = np.cumsum(complete_input_vector2[i])[:]
        p1 = np.cumsum(unpatterned_intervals[i])[:]
        p2 = np.cumsum(true_spikes_full[i])[:]
        p3 = np.cumsum(degraded_spikes_full)[:]
        for spike in complete_input_spikes:
            evecs[i].append(spike)
        vecstims[i].play(evecs[i])

        for spike in complete_input_spikes1:
            evecs1[i].append(spike)
        vecstims1[i].play(evecs1[i])
        for spike in complete_input_spikes2:
            evecs2[i].append(spike)
        vecstims2[i].play(evecs2[i])
...
#    stim_cons.append(nc)
    for ii in range(num_input):
       if ii == jj:
           nc = h.NetCon(vecstims1[ii], pc_ca3.synGroups['NMDA']['lucidum'][ii])
...
weight_changes = [[] for x in range(num_neurons)]
for jj in range(num_neurons):
    pc_ca3 = cell_list[jj]
    for ii in range(num_input):
...
t_lower_exposures = []
t_upper_exposures = []
t_lower_degradations = []
t_upper_degradations = []

for ii in range(num_exposures):
    t_lower_exposures.append(initializing_time + ii*(stim_length + stim_delay))
    t_upper_exposures.append(initializing_time + (ii)*(stim_length + stim_delay) + stim_length)
          
            
for jj in range(num_degradations):
    t_lower_degradations.append(initializing_time + (jj)*(stim_length + stim_delay))
    t_upper_degradations.append(initializing_time + (jj)*(stim_length + stim_delay) +  stim_length)

exposure_firing_rates = [[0 for i in range(num_neurons)] for j in range(num_exposures)]
degradation_firing_rates = [[0 for i in range(num_neurons)] for j in range(num_degradations)]

for jj in range(num_neurons):
    nc = net_cons[jj]
    exposure_spikes = [[] for x in range(num_exposures)]
    r_vec = nc.get_recordvec()
    for kk in range(len(r_vec)):
        spike = r_vec[kk]
        for ii in range(num_exposures):
            if spike > t_lower_exposures[ii] and spike <= t_upper_exposures[ii]:
                exposure_spikes[ii].append(spike)

                
    for ii in range(num_exposures):
        exposure_firing_rates[ii][jj] = (len(exposure_spikes[ii])/(stim_length/1000))
...
#    stim_cons.append(nc)
    for ii in range(num_input):
       if ii == jj:
           nc = h.NetCon(vecstims2[ii], pc_ca3.synGroups['NMDA']['lucidum'][ii])
           pc_ca3.synGroups['NMDA']['lucidum'][ii].tau1 = tau1
           pc_ca3.synGroups['NMDA']['lucidum'][ii].tau2 = tau2
           pc_ca3.synGroups['NMDA']['lucidum'][ii].tau3 = tau3
           pc_ca3.synGroups['NMDA']['lucidum'][ii].wtau2 = wtau2
...
for kk in range(num_neurons):
    ...
    for jj in range(num_neurons):
        if connection_matrix[jj][kk] == 1:
           ...
           post_cell.synGroups['AMPA']['radiatum'][jj].tau2 = 8.384766
           post_cell.synGroups['AMPA']['radiatum'][jj].e = 0
           nc.weight[0] = weight_array[kk][jj][-1]
           nc.delay = 0
           net_cons.append(nc)
...
for ii in range(num_neurons):
    v2.append(h.Vector())
    v2[-1].record(cell_list[ii].soma(0.5)._ref_v)
    
    t2.append(h.Vector())
    t2[-1].record(h._ref_t)
    
for ii in range(num_neurons):
    in_nc_spikes2.append(h.Vector())
    stim_cons[ii].record(in_nc_spikes[-1])
for ii in range(num_neurons):
    out_nc_spikes2.append(h.Vector())
    net_cons[ii].record(out_nc_spikes[-1])
...
for jj in range(num_neurons):
    nc = net_cons[jj]
    degradation_spikes = [[] for x in range(num_degradations)]
    r_vec = nc.get_recordvec()
    for kk in range(len(r_vec)):
        spike = r_vec[kk]
        for ii in range(num_degradations):
            if spike > t_lower_degradations[ii] and spike <= t_upper_degradations[ii]:
                degradation_spikes[ii].append(spike)
                
    for ii in range(num_degradations):
        degradation_firing_rates[ii][jj] = (len(degradation_spikes[ii])/(stim_length/1000))
