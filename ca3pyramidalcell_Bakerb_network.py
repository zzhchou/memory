...
def createSyn(synvars,sec_choice,seg_choice):
	...
	if synvars['type'] == "Log_E2Syn":
		syn = h.Log_E2Syn(sec_choice(seg_choice))
	if synvars['type'] == "Add_E2Syn":
		syn = h.Add_E2Syn(sec_choice(seg_choice))
	if synvars['type'] == "Mult_E2Syn":
...
def addReducedSynapses(cell, num_input, num_cells):
    for syntype in cell.synGroups:
        synchoice = cell.synvars
        if syntype == 'NMDA':
            synchoice['type'] = 'E3_NMDA'
        if syntype == 'AMPA':
            synchoice['type'] = 'Log_E2Syn'
            synchoice['type'] = 'Add_E2Syn'
            synchoice['type'] = 'Mult_E2Syn'
#            synchoice['type'] = 'E2'
